#!/usr/bin/python3
# -*- coding: utf-8 -*-
# Created by: python.exe -m py2exe ..\src\run_imgur_switcher.py -W setup.py
...
class Target(object):
    ...
    def __init__(self, **kw):
        ...
        self.product_name = "ImgurSwitcher"
        self.licence = "GPL3"
...
ImgurSwitcher = Target(
    # We can extend or override the VersionInfo of the base class:
    # version = "1.0",
    file_description = "The stand-alone Windows 64-bit ImgurSwitcher executable.",
    comments = "Does not work on Itanium-based processors.",
    # internal_name = "spam",

    script="run_imgur_switcher.py", # path of the main script

    # Allows to specify the basename of the executable, if different from '..\src\run_imgur_switcher'
    dest_base = "imgurswitcher",

    # Icon resources:[(resource_id, path to .ico file), ...]
    # icon_resources=[(1, r"..\src\run_imgur_switcher.ico")]

    other_resources = [(RT_MANIFEST, 1, (manifest_template % dict(prog="run_imgur_switcher", level="asInvoker")).encode("utf-8")),
    # for bitmap resources, the first 14 bytes must be skipped when reading the file:
    #                    (RT_BITMAP, 1, open("bitmap.bmp", "rb").read()[14:]),
                      ]
    )

Mydata_files = [('data', ['imgurswitcher/data/config.cfg', 'ImgurSwitcher/data/default.jpg'])]
...
##    dll_excludes = "MSVCP90.dll mswsock.dll powrprof.dll".split(),
    ...
    bundle_files=3, # doesn't work if this isn't 3
