...
%wrapper %{
  ...
  LRESULT CALLBACK cLLKeyboardCallback(int code, WPARAM wParam, LPARAM lParam) {
    ...
    // get the desktop window, which is apparently this shell window
    // because the real desktop window as returned by GetDesktopWindow()
    // runs underneath the window returned by GetShellWindow()
    //desktopHwnd = GetShellWindow();

    // If the desktop window isn't the foreground window,
    // we don't want to call the callback
    //if(desktopHwnd != hwnd)
    //{
    //  call_callback = 0;
    //}
